...
tableStyle <- "multiline" # for .docx use "multiline" otherwise "rmarkdown"
tableResults <- "asis"
...
ggplot(crashes[!is.na(crashes$hour) & crashes$alcohol == TRUE, ]
       , aes(hour, weight = count, fill = as.character(excelPalette[1]))) + 
  geom_bar(binwidth = 1) +
...
ggplot(crashes[!is.na(crashes$hour) & crashes$alcohol == TRUE, ]
       , aes(hour, weight = countCrashUrbanHour, fill = as.character(excelPalette[1]))) + 
  geom_bar(binwidth = 1) +
  scale_x_discrete(breaks = seq(0,24,4)) +
  facet_grid(urbanRural ~ .) +
...
lines(density(x, bw = 200, ), col = excelPalette[1], lwd = 2)
...
ggplot(x, aes(age, weight = countAgeGroupPopulation, fill = as.character(excelPalette[1]))) +
...
x <- drivers[!is.na(drivers$age) & drivers$fault == TRUE & drivers$alcohol == TRUE, ]
ggplot(x, aes(ethnicity, weight = countUrbanEthnicity, fill = ethnicity)) +
  geom_bar() + 
  facet_grid(. ~ urbanRural) +

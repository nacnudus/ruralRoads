...
x <- dcast(crashes[!is.na(crashes$hour) & crashes$alcohol == TRUE, ]
           , urbanRural + alcohol ~ hour, length, value.var = "count")
colnames(x)[1:2] <- c("Urban/Rural\nRoad/StateHighway", "Alcohol a Factor")
pandoc.table(x
...
colnames(x)[1:2] <- c("Hour of Day", "Rural", "Urban")
pandoc.table(x
...
pandoc.table(x
             , caption = "Drink-Drivers in Crashes per Population by age in 5-year bands"
             ,style = tableStyle
             , split.tables = Inf)
```
...
pandoc.table(x
             , caption = "Drink-drivers in crashes per population by urban/rural and ethnicity"
             ,style = tableStyle
             , split.tables = Inf)
```

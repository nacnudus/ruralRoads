...
```{r populationCrashRateTable2, results = tableResults, cache = TRUE}
x <- dcast(crashes[crashes$urbanRuralHighway!="highway", ], urbanRural + alcohol ~ year, sum, value.var = "countPopulation")
colnames(x)[1:2] <- c("Urban/Rural\nRoad/StateHighway", "Alcohol a Factor")
pandoc.table(x
...
```{r populationCrashRate2, cache = TRUE}
plotLabels <- list(
  "TRUE" = "alcohol a factor"
  , "FALSE" = "alcohol not a factor"
  )
...
ggplot(crashes[crashes$urbanRuralHighway!="highway", ], aes(year, weight = countPopulation, group = urbanRural
                    , fill = urbanRural)) + 
  geom_bar(position = "dodge", colour = "black") +
  facet_grid(. ~ alcohol, labeller = plotLabeller) +
  ggtitle("Crashes per 1000 people\nby urban/rural and whether\nalcohol was a factor\nexcluding state highways") +

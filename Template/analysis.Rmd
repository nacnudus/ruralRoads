...
pandoc.table(SummaryLocal[c("urban", "rural"), c("road", "highway")]
...
pandoc.table(data.frame("Region" = c("urban", "rural")
                        , Population = SummaryLocal[c("urban", "rural")
                                                  , "population"])
...
x <- dcast(crashes[!is.na(crashes$hour) & crashes$alcohol == TRUE, ]
           ,  hour ~ urbanRural, length, value.var = "count"
           , drop = FALSE)
...
x <- dcast(crashes[!is.na(crashes$hour) & crashes$alcohol == TRUE, ]
           , urbanRural + weekday + hour ~ ., sum
           , value.var = "countCrashUrbanWeekdayHour3", drop = FALSE)
# group into threes
...
x <- dcast(drivers[!is.na(drivers$age) &
                     !is.na(drivers$urbanRural) &
                     drivers$fault == TRUE & drivers$alcohol == TRUE, ]
...
```{r age, cache = TRUE}
x <- drivers[!is.na(drivers$age) & !is.na(drivers$urbanRural) & drivers$fault == TRUE & drivers$alcohol == TRUE, ]
...
x <- ddply(drivers[!is.na(drivers$urbanRural) & drivers$fault == TRUE & drivers$alcohol == TRUE, ]
...
x <- dcast(mDataLocal[mDataLocal$variable %in% c("European", "NZ Maori"
